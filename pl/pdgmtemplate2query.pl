...
my $queryfile = "sparql/pdgms/$langname";
...
my ($lang, $qprops) = split(/,/, $queryprops, 2);
my $Lang = uc($lang);
my $Langname = uc($langname);
my @select = split(/,\s* /, $qprops);
my $selection = "";
foreach my $queryprop (@select)
 {  
	#if ($specifiedprops !~ /$queryprop.":"/)
	if ( $specifiedprops{$qprop} !~ /\w/)
	{
		$selection .= "?".$qprop." "; 
		push( @queryproplist, $qprop );
	}
}
#my $aamalang  = "";
#$aamalang = $langname;
