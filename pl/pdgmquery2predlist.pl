...
# aa directory. It is invoked by doc-prop-val-queries.sh. It takes  
# each query* file (i.e., each SPARQL query tsv output file  of format  
# pred1 \t pred2 \t pred3 \t pred4) and transforms it  into a txt file in which all instances 
# of the third pred which occur with the first two are gathered into a tab-separated list, 
# list, of format pred1 \t pred2 \t pred3list. The envisaged values for the preds are: 
# prop \t val \t lang , val \t prop \t lang \, lang \t prop \t val,
# and prop \t \t  lang (for simple prop list, including token*).
